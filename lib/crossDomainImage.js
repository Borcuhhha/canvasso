/**
 * Created by Islam on 03.12.2016.
 */
...
function fromURL(url, callback){
	...
	image.onload = function(){
		...
		CrossDomainImage.fromImage(image, callback);
	};

}

function fromImage(img, callback){
	if(!img.complete || (parseHostname(img.src) != hostname && (img.crossOrigin != 'anonymous' || !testCrossDomainSupport(img)))){
		CrossDomainImage.fromURL(img.src, callback); return;
	}
